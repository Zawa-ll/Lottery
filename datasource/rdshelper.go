...
func (rds *RedisConn) Do(commandName string,
	...
	// change conenction status and put conn back to pool
	defer conn.Close()
	...
	if rds.showDebug {
		fmt.Printf("[redis] [info] [%dus]cmd=%s, err=%s, args=%v, reply=%s\n",
			(t2-t1)/100, commandName, err, args, reply)
	}
	return reply, err
}
...
func NewCache() *RedisConn {
	pool := redis.Pool{
		Dial: func() (redis.Conn, error) {
			c, err := redis.Dial("tcp", fmt.Sprintf("%s:%d", conf.RdsCache.Host,
				conf.RdsCache.Port))
			if err != nil {
				log.Fatal("reshelper.NewCache Dial error=", err)
				return nil, err
			}
			return c, nil
		},
